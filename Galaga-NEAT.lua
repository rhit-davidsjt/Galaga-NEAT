-- Galaga-NEAT, adapted from MarI/O by SethBling
-- Feel free to use this code, but please do not redistribute it.
-- Intended for use with the BizHawk emulator and Galaga ROM.
...
function getTile(dx, dy) -- probably need a different system
	-- if gameinfo.getromname() == "Super Mario World (USA)" then
...
function getSprites() -- oof oww memory locations
	-- if gameinfo.getromname() == "Super Mario World (USA)" then
	-- 	local sprites = {}
	-- 	for slot=0,11 do
...
function getInputs()
	...
	for dy=-BoxRadius*16,BoxRadius*16,16 do
		for dx=-BoxRadius*16,BoxRadius*16,16 do
			...
			for i = 1,#extended do
				distx = math.abs(extended[i]["x"] - (playerX+dx))
				disty = math.abs(extended[i]["y"] - (playerY+dy))
				if distx < 8 and disty < 8 then
					inputs[#inputs] = -1
				end
			end
	...
	--mariovx = memory.read_s8(0x7B) not commented by Jack
